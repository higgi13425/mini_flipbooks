...
Negation is done be preceding with ! ,
so !is.na(variable) produces a logical vector with
TRUE for all non-missing value rows.

You can **R**emember that filte**R** is for selecting **R**ows because it ends with an **R**.

Example 1: filter covid_dates data for missing dates.
...
`r chunk_reveal("filter1", 
    break_type = "auto", 
    title = "Example 1: Filter Rows for Missing Dates")`
...
`r chunk_reveal("filter2", 
    break_type = "auto", 
    title = "Example 2: Filter Rows for Non-Missing Dates")`
...
`r chunk_reveal("filter3", 
    break_type = "auto", 
    title = "Example 3: Filter Rows for Non-Missing Dates")`
...
`r chunk_reveal("filter4", 
    break_type = "auto", 
    title = "Example 4: Filter Rows for Complete Cases - with no NAs")`
